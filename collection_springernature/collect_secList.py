...
class Collector(threading.Thread):
    ...
    def collectOnePage(self, url):
        ...
        try:
            response = requests.request(method = 'GET', url = url, proxies = proxies, headers = headers, timeout = 30)
        except:
            ...
            for e in root.xpath(".//div[@class= 't-alphaindex columns hidehardwiredtitles']/ul//li/span[@class = 't-field t-new t-index-main']/a[@href]"):
                title = getValueFromPath(e.xpath("./text()"))
                article_url = getValueFromPath(e.xpath("./@href"))
    ...
    def run(self):
        while True:
            ...
            self.coll.update_one(
                ...
                {
                    '$setOnInsert' : {'article_url' : data['article_url'], 'title' : data['title'], 'failed_count' : 0, 'statu' : 0, 'create_time' : current_time}, 
...
def test():
    response = requests.request(method = 'GET', headers = headers, url = 'https://publications.arl.org/9ishf/title_index?indexLetter=ALL&indexField=IndexTitle&&screen=0', proxies = proxies, timeout = 30)
    response.encoding = 'utf-8'
    root = et.HTML(response.text)
    ...
    for e in root.xpath(".//div[@class= 't-alphaindex columns hidehardwiredtitles']/ul//li/span[@class = 't-field t-new t-index-main']/a[@href]"):
        title = getValueFromPath(e.xpath("./text()"))
        article_url = getValueFromPath(e.xpath("./@href"))
        print({'title' : title, 'article_url' : article_url})
